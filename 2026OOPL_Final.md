...
1. 啟動後進入首頁，設有 Music player、Settings、Handbook 及 About 等功能選單。其中 Music player 支援 7 首背景音樂點播；首頁 BGM 預設關閉，可於設定中啟用。
...
13. 遊戲中按 M 鍵（或於首頁點選 Settings）可以呼叫設定彈窗，調整音樂音量並暫停遊戲。設定彈窗中設計了橫排三個開關控制（Cheat Mode、Home BGM、Result BGM），兩處設定頁面狀態全時同步。當進入結算畫面時，若 Result BGM 開關開啟，則 1-4 關播放結算音樂A，第 5 關播放結算音樂B。
...
#### 10. 音樂播放器與場景 BGM 控制系統

- **Music player 音樂點播器**：支援多首背景音樂選取，並在點播清單中加入新曲目 `Man Ya (Mixed Voice).wav`，優化行距排版以容納更多曲目。
- **設定開關橫排並列與狀態同步**：首頁與遊戲內 Settings 彈窗均實作了 Volume 調整、Cheat Mode、Home BGM、Result BGM 四大設定。將三大開關（Cheat Mode, Home BGM, Result BGM）改為同一橫排並列顯示（X 座標為 -140, 0, 140），成功將設定彈窗高寬比優化並控制在精巧的 320 像素高度內，同時確保首頁與遊戲內設定的即時同步。
- **關卡專屬結算音樂與析構控制**：結算畫面（Result）支援依據關卡播放不同版本的結算音樂（1-4 關為無人聲版，5 關為有人聲版），且受 Result BGM 開關控制。在轉場進入遊戲、退出或返回首頁時，皆會主動呼叫 `m_Home.reset()` 或 `m_GameSession.reset()`，除了釋放多餘資源，也避免了 C++ 中 unique_ptr 賦值順序造成新畫面音樂被舊析構函式 halt 的問題。
...
| 結算畫面回到首頁時，首頁背景音樂未能播放 (或被異常靜音) | 這是因為 C++ 中 `std::unique_ptr` 賦值時會先執行右側的建構子（播放音樂），再執行左側舊物件的析構（停止音樂）。解決方法是在重新賦值前主動呼叫 `reset()` 釋放舊物件。 |
| 設定彈窗中若將 Volume、Cheat、Home BGM、Result BGM 直排堆疊，會使彈窗過高且雜亂 | 將 Cheat Mode, Home BGM, Result BGM 三個開關水平並列在同一橫排（X 分佈於 -140, 0, 140），成功使彈窗尺寸控制在原有的 500x320 像素，並保持清晰整潔。 |
