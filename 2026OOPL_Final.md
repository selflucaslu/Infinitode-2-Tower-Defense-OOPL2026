...
12. 第 1 關完成所有波次後會直接進入第 2 關。第 2 關以後完成一輪後會進入無限循環，敵人血量乘上 `1.2^loopCount`，速度乘上 `1.1^loopCount`；在 F1 啟用作弊模式後按 P 鍵或是完成第一輪循環後，若在第 2~4 關會顯示前往下一關按鈕，若在第 5 關則會顯示直接結束本局進入結算畫面的按鈕。
13. 遊戲中按 M 鍵可以呼叫設定彈窗調整音樂音量並暫停遊戲。
14. 按 F1 鍵可以開啟或關閉作弊模式，啟用後可以使用 G（加100金幣）、H（加1條命）、P（跳關）等作弊按鍵。
