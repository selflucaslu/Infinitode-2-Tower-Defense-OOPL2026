...
12. 第 1 關完成所有波次後會直接進入第 2 關。第 2 關以後完成一輪後會進入無限循環，敵人血量乘上 `1.2^loopCount`，速度乘上 `1.1^loopCount`；若目前關卡存在下一關，完成第一輪循環後會顯示前往下一關按鈕，玩家也可在 F1 啟用作弊模式後，按 P 提前顯示同一個測試用切關按鈕。
