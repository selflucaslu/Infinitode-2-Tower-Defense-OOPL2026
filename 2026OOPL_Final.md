...
1. 啟動後進入首頁，只有 New game 是可操作的開始按鈕；其他首頁按鈕目前是展示用 disabled 按鈕。
...
5. 按 X 可以拆除滑鼠所在格子的塔，退還該塔建造費用的一半。
6. WASD 可以平移地圖，滑鼠右鍵拖曳也可以平移地圖，滑鼠滾輪可以縮放地圖。縮放倍率限制在 0.1 到 3.0。
...
11. 敵人抵達終點會依敵人設定的 `damageToBase` 扣基地生命；基地生命歸零時遊戲結束並進入結算畫面。
12. 第 1 關完成所有波次後會直接進入第 2 關。第 2 關以後完成一輪後會進入無限循環，敵人血量乘上 `1.2^loopCount`，速度乘上 `1.1^loopCount`；若目前關卡存在下一關，完成第一輪循環後會顯示前往下一關按鈕，玩家也可按 P 提前顯示同一個測試用切關按鈕。
...
| 17 | 具有 debug mode 的功能 | 部分完成：有 FPS、P 鍵測試切關按鈕、debug enemy 入口與 log，但沒有獨立 debug mode 選單 |
| 18 | 解決專案上所有 Memory Leak 的問題 | 使用 RAII、`unique_ptr`、`shared_ptr` 管理資源；本次未執行 sanitizer，無法宣稱已以工具證明零 leak |
