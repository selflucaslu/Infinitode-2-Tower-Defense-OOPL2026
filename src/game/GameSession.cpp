...
GameSession::GameSession(int levelNumber)
  ...
  static constexpr std::array<EnemyTypeId, 18> preloadEnemyTypes = {
      EnemyTypeId::Regular, EnemyTypeId::Fast,  EnemyTypeId::Strong,
      EnemyTypeId::Heli,    EnemyTypeId::Jet,   EnemyTypeId::Armored,
      EnemyTypeId::Healer,  EnemyTypeId::Toxic, EnemyTypeId::Icy,
      EnemyTypeId::Fighter,
      // EnemyTypeId::Mage,
      // EnemyTypeId::Camole,
      // EnemyTypeId::Drone,
  ...
  towerHpText = std::make_shared<Util::Text>(kHudFontPath, kHudFontSize, "0",
                                             Util::Color::FromRGB(255, 96, 96));
  ...
  goldText = std::make_shared<Util::Text>(kHudFontPath, kHudFontSize, "0",
                                          Util::Color::FromRGB(255, 216, 80));
  ...
  waveText = std::make_shared<Util::Text>(kHudFontPath, kHudFontSize, "0",
  ...
  nextLevelButtonObject->SetDrawable(
      atlasLoader->getImage("icon-step-forward"));
...
bool GameSession::placeTower(int gridX, int gridY, TowerId towerId) {
  ...
  if (gold < def.buildCost) {
    return false;
  }

  // 2) 交給 TowerManager 檢查格子是否合法並實際建塔。
  if (!towerManager->placeTower(gridX, gridY, towerId)) {
    return false;
  }
...
void GameSession::setSelectedTower(TowerId id) {
  if (m_SelectionPanel) {
    m_SelectionPanel->setSelectedTower(id);
  }
}

TowerId GameSession::getSelectedTower() const {
  if (m_SelectionPanel) {
    return m_SelectionPanel->getSelectedTower();
  }
  return TowerId::Basic;
}

std::optional<TowerId> GameSession::hitTestSelectionPanel(float screenX,
                                                          float screenY) const {
  if (!m_SelectionPanel)
    return std::nullopt;
...
int GameSession::getLoopCount() const { return loopCount; }
...
bool GameSession::hitTestNextLevelButton(float screenX, float screenY) const {
  if (!canAdvanceToNextLevel || !nextLevelButtonObject)
    return false;
...
// -------------------- 每幀流程 --------------------
void GameSession::update(float deltaTime) { update(deltaTime, deltaTime); }
...
void GameSession::update(float deltaTime, float rawDeltaTime) {
  ...
  // 5) 結算本幀死亡敵人與抵達終點的敵人。
  const EnemyManager::FrameResolveResult frameResult =
      enemyManager->resolveAndRemoveDeadAndReached();
...
void GameSession::dispatchEnemiesByTimer() {
  ...
  if (waveTimer >= waveConfig.prepTime) {
    ...
    if (groupIndex < groupCount) {
      ...
      const int scaledHp = static_cast<int>(
          static_cast<float>(config.maxHealth) * spawnGroup.hpMultiplier);

      // 5) 第一隻：等到 prepTime + startDelay。
      if (groupSpawned == 0) {
        if (waveTimer >= waveConfig.prepTime + spawnGroup.startDelay) {
          enemyManager->spawnEnemiesAt(spawnGroup.spawnPointIndices,
                                       scaledSpeed, config.moveType, scaledHp,
                                       config.damageToBase, config.rewardGold,
                                       config.spriteId);
          groupSpawned += 1;
          groupTimer = 0.0F;
        }
...
void GameSession::updateTowerDisplay() {
  ...
  while (towerBaseObjects.size() < towers.size()) {
    std::shared_ptr<Util::GameObject> towerBaseObject =
    ...
    std::shared_ptr<Util::GameObject> towerWeaponObject =
        std::make_shared<Util::GameObject>();
  ...
  for (std::size_t i = 0; i < towers.size(); ++i) {
    ...
    const std::shared_ptr<Util::GameObject> &towerBaseObject =
        towerBaseObjects[i];
    const std::shared_ptr<Util::GameObject> &towerWeaponObject =
        towerWeaponObjects[i];
    ...
    } else if (auto pos2 = weaponId.rfind(baseSuffix);
               pos2 != std::string::npos) {
      weaponId.replace(pos2, baseSuffix.size(), "-weapon");
    }
    towerWeaponObject->SetDrawable(atlasLoader->getImage(weaponId));

    const std::optional<glm::vec2> worldPos =
        map->gridToWorld(tower.GetGridX(), tower.GetGridY());
    if (!worldPos.has_value())
      continue;

    towerBaseObject->m_Transform.scale = {kTowerScale * currentScale,
                                          kTowerScale * currentScale};
    towerWeaponObject->m_Transform.scale = {kTowerScale * currentScale,
                                            kTowerScale * currentScale};
...
void GameSession::updateProjectileDisplay() {
  // 1) 取得目前所有子彈與地圖縮放倍率。
  const std::vector<TowerManager::Projectile> &projectiles =
      towerManager->getProjectiles();
  const float currentScale = map->getCurrentScale();

  // 2) 如果子彈變多，就補上顯示物件。
  while (projectileObjects.size() < projectiles.size()) {
    std::shared_ptr<Util::GameObject> projectileObject =
        std::make_shared<Util::GameObject>();
  ...
  for (std::size_t i = 0; i < projectiles.size(); ++i) {
    ...
    const std::shared_ptr<Util::GameObject> &projectileObject =
        projectileObjects[i];

    projectileObject->m_Transform.scale = {kProjectileScale * currentScale,
                                           kProjectileScale * currentScale};

    const std::optional<glm::vec2> worldPos =
        map->gridToWorld(projectile.x, projectile.y);
    if (!worldPos.has_value())
      continue;
...
void GameSession::updateHudDisplay() {
  ...
  if (!towerHpText || !goldText || !waveText || !towerHpIconObject ||
      !goldIconObject || !waveIconObject || !towerHpTextObject ||
      !goldTextObject || !waveTextObject)
    return;

  // 2) 更新 HUD 數字，不顯示中文 label。
  towerHpText->SetText(std::to_string(baseHp));
  goldText->SetText(std::to_string(gold));
  const int displayWave =
      (waveCount % static_cast<int>(baseSpawnSchedule.size())) + 1;
  waveText->SetText(std::to_string(loopCount + 1) + "-" +
                    std::to_string(displayWave));
  ...
  const glm::vec2 towerIconSize =
      atlasLoader->getImage("game-ui-health-icon")->GetSize();
  const glm::vec2 goldIconSize =
      atlasLoader->getImage("game-ui-coin-icon")->GetSize();
  ...
  towerHpIconObject->m_Transform.translation = {
      towerGroupLeft + towerIconWidth * 0.5F, hudCenterY};
  towerHpTextObject->m_Transform.translation = {
      towerHpIconObject->m_Transform.translation.x + towerIconWidth * 0.5F +
          kHudGap + towerTextSize.x * 0.5F,
      hudCenterY};

  // 6) 上方置中顯示波次 icon + 數字。
  const float waveGroupLeft = -waveGroupWidth * 0.5F;
  waveIconObject->m_Transform.translation = {
      waveGroupLeft + waveIconWidth * 0.5F, hudCenterY};
  waveTextObject->m_Transform.translation = {
      waveIconObject->m_Transform.translation.x + waveIconWidth * 0.5F +
          kHudGap + waveTextSize.x * 0.5F,
      hudCenterY};

  // 7) 右上角顯示金幣 icon + 數字。
  const float goldGroupLeft = halfWindowWidth - hudSideMargin - goldGroupWidth;
  goldIconObject->m_Transform.translation = {
      goldGroupLeft + goldIconWidth * 0.5F, hudCenterY};
...
void GameSession::updateNextLevelButtonDisplay() {
  ...
  const glm::vec2 iconSize =
      atlasLoader->getImage("icon-step-forward")->GetSize();

  nextLevelButtonObject->m_Transform.translation = {
      halfWindowWidth - kNextLevelButtonMargin - kNextLevelButtonSize * 0.5F,
      -halfWindowHeight + kNextLevelButtonBottomOffset +
          kNextLevelButtonSize * 0.5F};
  nextLevelButtonObject->m_Transform.scale = {
      kNextLevelButtonSize / iconSize.x, kNextLevelButtonSize / iconSize.y};
}

void GameSession::spawnDebugEnemy(
    EnemyTypeId enemyTypeId, const std::vector<int> &spawnPointIndices) const {
  // 1) 取得指定敵人類型的設定。
  const EnemyTypeConfig &config = getEnemyTypeConfig(enemyTypeId);

  // 2) 直接在指定出生點生成敵人，方便測試地圖路徑與戰鬥。
  enemyManager->spawnEnemiesAt(spawnPointIndices, config.speed, config.moveType,
                               config.maxHealth, config.damageToBase,
                               config.rewardGold, config.spriteId);
}
